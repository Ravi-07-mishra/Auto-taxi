...
import {
  Users, Car, Clock, Mail, Phone, MapPin, Menu, X, Star, ChevronLeft, ChevronRight,
} from "lucide-react";
  ...
  return (
    <div className="relative min-h-screen">
      <video autoPlay loop muted className="fixed top-0 left-0 w-full h-full object-cover z-0">
        <source src="/HomePage.mp4" type="video/mp4" />Your browser does not support the video tag.
      </video>
      ...
      <div className="relative z-20">
        ...
        <nav className="fixed w-full z-30 top-0 left-0 p-4 bg-black bg-opacity-50">
          ...
          {["a", "u", "t", "o", "-", "d", "r", "i", "v", "e"].map((letter, index) => (
            ...
            <div className="hidden md:flex gap-8">
              ...
              <button onClick={() => { topDriversRef.current?.scrollIntoView({ behavior: "smooth" }); setIsMenuOpen(false); }} className="text-white hover:text-[#cbe557] transition-colors">Trusted Drivers</button>
              <NavLink href="/OurServices" text="Our Services" currentPath="/Home" />
              <button onClick={scrollToContact} className="bg-[#cbe557] text-black px-4 py-2 rounded-lg hover:bg-opacity-80 transition-all">Contact Us</button>
            </div>
        ...
        {isMenuOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-90 z-40 flex flex-col items-center justify-center space-y-6">
            <button className="absolute top-4 right-4 text-white" onClick={toggleMenu}><X size={24} /></button>
            ...
            <button onClick={() => { scrollToContact(); toggleMenu(); }} className="bg-[#cbe557] text-black px-4 py-2 rounded-lg hover:bg-opacity-80 transition-all">Contact Us</button>
          </div>
        )}
        ...
        <section className="min-h-screen flex items-center justify-center px-4">
          <div className="text-center text-white">
            <h2 className="text-3xl sm:text-4xl md:text-6xl font-bold mb-8" style={{ fontFamily: "Merriweather" }}>Book Drives or Start Driving</h2>
            <button onClick={handleGetStartedClick} className="bg-[#cbe557] text-gray-800 font-bold py-3 px-6 sm:px-8 rounded-lg shadow-lg hover:bg-[#b8d93e] hover:shadow-xl transition-all">Get Started</button>
          </div>
        </section>
        ...
        <section className="py-20 bg-black bg-opacity-70">
          <div className="max-w-screen-xl mx-auto px-4 w-full">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-white text-center">
              {[{ icon: Users, value: "20k+", label: "Users" }, { icon: Car, value: "100+", label: "Drivers" }, { icon: Clock, value: "50k+", label: "Hours Driven" }].map((stat, i) => (
                <div key={i}>
        ...
        <section className="py-16 bg-black bg-opacity-80" ref={topDriversRef}>
          <div className="max-w-screen-xl mx-auto px-4 w-full">
            <h2 className="text-3xl md:text-4xl font-bold mb-12 text-center text-white">Our Top-Rated Drivers</h2>
            {loadingDrivers ? (
              ...
              <div className="relative max-w-4xl mx-auto">
                <Carousel showArrows showStatus={false} showThumbs={false} infiniteLoop autoPlay interval={5000}
                  renderArrowPrev={(onClickHandler, hasPrev, label) => (
                    <button onClick={onClickHandler} disabled={!hasPrev} aria-label={label} className="absolute left-0 top-1/2 z-10 -translate-y-1/2 bg-[#cbe557] bg-opacity-80 p-2 rounded-full hover:bg-opacity-100">
                      <ChevronLeft className="h-6 w-6 text-black" />
                    </button>
                  )}
                  renderArrowNext={(onClickHandler, hasNext, label) => (
                    <button onClick={onClickHandler} disabled={!hasNext} aria-label={label} className="absolute right-0 top-1/2 z-10 -translate-y-1/2 bg-[#cbe557] bg-opacity-80 p-2 rounded-full hover:bg-opacity-100">
                      ...
                      <div className="bg-white bg-opacity-10 backdrop-blur-md rounded-xl p-6 shadow-lg border border-[#cbe557] border-opacity-30">
                        <div className="flex flex-col items-center">
                          <div className="relative mb-6">
                            <img src={`http://localhost:3000/${driver.profileImage}`} alt={driver.name} className="w-32 h-32 rounded-full object-cover border-4 border-[#cbe557]" onError={(e) => { e.target.src = "/default-driver.jpg" }} />
                          ...
                          <div className="flex justify-center mt-2">
                            {[...Array(5)].map((_, i) => (
                              <Star key={i} className={`h-5 w-5 ${i < Math.round(driver.avgRating) ? "fill-[#cbe557] text-[#cbe557]" : "text-gray-400"}`} />
                            ))}
                          </div>
                        </div>
                      </div>
        ...
        <section ref={contactRef} className="py-20 bg-black bg-opacity-70 text-white">
          <div className="max-w-screen-xl mx-auto px-4 w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 justify-items-center">
              <div className="flex items-center"><Mail className="mr-2 text-[#cbe557]" /><span>info@auto-drive.com</span></div>
              <div className="flex items-center"><Phone className="mr-2 text-[#cbe557]" /><span>+1 (555) 123-4567</span></div>
              <div className="flex items-center"><MapPin className="mr-2 text-[#cbe557]" /><span>123 Drive St, City, State 12345</span></div>
            </div>
          </div>
        </section>
        ...
        {isCardVisible && (
          <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50 p-4">
            ...
            <button onClick={handleCloseCard} className="absolute top-4 right-4 bg-red-500 text-white font-bold py-2 px-4 rounded-full hover:bg-red-600 transition-all">Close</button>
          </div>
        )}
      </div>
    </div>
