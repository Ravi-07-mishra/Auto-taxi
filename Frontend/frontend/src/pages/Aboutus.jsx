...
const Feature = ({ icon, title, description }) => (
  <motion.div
    className="p-8 bg-white/20 backdrop-blur-lg rounded-3xl shadow-lg flex flex-col items-center text-center transition-transform transform hover:scale-105 border border-white/30"
  ...
  >
    ...
    <p className="text-gray-200 mt-3 text-lg">{description}</p>
  </motion.div>
);

const AboutUs = () => {
  return (
    <div className="relative text-white min-h-screen py-20 px-4 md:px-8 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto text-center">
        <motion.h1
          className="text-5xl md:text-6xl font-extrabold text-indigo-400 drop-shadow-lg"
          initial={{ opacity: 0, y: -20 }}
        ...
        <motion.p
          className="mt-4 text-xl md:text-2xl text-gray-300 drop-shadow-sm"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      {/* Mission Statements */}
      <div className="relative z-10 max-w-5xl mx-auto mt-12 grid md:grid-cols-2 gap-8">
        <motion.p
          className="text-gray-300 text-lg leading-relaxed bg-white/10 backdrop-blur-lg p-6 rounded-3xl border border-white/20"
      ...
      {/* Features Section */}
      <motion.h2
        className="relative z-10 mt-16 text-4xl font-bold text-center text-indigo-400"
        initial={{ opacity: 0, y: -20 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6 }}
        viewport={{ once: true }}
      ...
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mt-12 max-w-6xl mx-auto">
        <Feature 
          icon={<DirectionsCar />} 
      ...
      <motion.div
        className="relative z-10 bg-white/10 backdrop-blur-lg p-12 rounded-3xl shadow-xl text-center mt-16 max-w-5xl mx-auto border border-white/20"
      ...
      >
        ...
        <div className="mt-8 flex flex-col sm:flex-row justify-center gap-6">
          <motion.button
            className="bg-indigo-500 hover:bg-indigo-600 text-white px-8 py-4 rounded-xl text-xl font-medium transition-all"
            whileHover={{ scale: 1.05 }}
          >
            Download App
          </motion.button>
          <motion.button
            className="border-2 border-indigo-500 hover:bg-indigo-500 text-white px-8 py-4 rounded-xl text-xl font-medium transition-all"
            whileHover={{ scale: 1.05 }}
          >
            Become a Driver
          </motion.button>
      ...
      {/* Contact Section */}
      <motion.div
        className="relative z-10 text-center mt-16 bg-white/10 backdrop-blur-lg p-8 rounded-3xl border border-white/20 max-w-2xl mx-auto"
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6 }}
        viewport={{ once: true }}
